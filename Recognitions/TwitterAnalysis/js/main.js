    ...
    var s =
        '<div class="i4twitter_item">'
            ...
            + '<div class="ProfileCard-details">'
                + '<div class="i4twitter_name">' + actor.displayName + '</div>'
                + '<div class="i4twitter_user">@' + actor.preferredUsername + '</div>'
                + '<div class="i4twitter_body">' +
                    ...
                    + '<div>'
                        + '<a href="javascript:showSection(\'tweet\', \'insight\', ' + id + ')">'
                            + '<span id="i4twitter_insight_link_' + id + '" class="i4twitter_insight">Sentiment</span>'
                        + '</a>'
                        + '<span class="i4twitter_insight"> </span>'
                        + '<a href="javascript:showSection(\'insight\', \'tweet\', ' + id + ')">'
                            + '<span id="i4twitter_tweet_link_' + id + '" class="i4twitter_tweet">Tweet Info</span>'
                        + '</a>'
                    + '</div>'
                + '</div>'
            + '</div>'
...
function countTweets(term) {
    if (term != "") {
        ...
        spinnerStart();
        $.ajax({
            url: "https://cdetestapp.mybluemix.net/api/count",
            type: 'GET',
...
function searchTweets(term) {
    if (term != "") {
        ...
        spinnerStart();
        $.ajax({
            url: "https://cdetestapp.mybluemix.net/api/search",
            type: 'GET',
            contentType: 'application/json',
            data: {
                q: term
